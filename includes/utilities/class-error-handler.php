...
class Error_Handler {
	...
	protected function should_pause_batch_operation() {
		...
		if ( $max_execution_time > 0 ) {
			$script_start_time = isset( $_SERVER['REQUEST_TIME_FLOAT'] ) ? (float) wp_unslash( $_SERVER['REQUEST_TIME_FLOAT'] ) : (float) wp_unslash( $_SERVER['REQUEST_TIME'] ?? 0 ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput -- Server-set request time, used only for arithmetic, never output.
